...
// Realex Payments Hook
// This hook provides access to the Realex Payments library

// Type declaration for RealexHpp (third-party library)
declare const RealexHpp: any;

const useRealex = (): typeof RealexHpp => {
  ...
  const RealexHpp = (function () {
    ...
    let hppUrl = import.meta.env.VITE_GLOBAL_PAY_SERVICE_URL;

    var randomId = randomId || Math.random().toString(16).substr(2, 8);

    const setHppUrl = function (url) {
      hppUrl = url;
    };

    let mobileXSLowerBound = 360;
    const setMobileXSLowerBound = function (lowerBound) {
    ...
    var internal = {
      ...
      createForm: function (doc, token, ignorePostMessage) {
        ...
        if (ignorePostMessage) {
          form.appendChild(
            internal.createFormHiddenInput(
      ...
      receiveMessage: function (lightboxInstance, merchantUrl, isEmbedded) {
        return function (event) {
          ...
          if (
            event.data &&
            (evtdata = internal.decodeAnswer(event.data)).iframe
          ) {
            if (!isMobileNewTab()) {
        ...
        return {
          lightbox: function () {
            if (isMobileNewTab()) {
              ...
              const temp = internal.createIFrame(overlayElement, token);
      ...
      return {
        ...
        init: function (idOfLightboxButton, merchantUrl, serverSdkJson) {
          ...
          // Sets the event listener on the PAY button. The click will invoke the lightbox method
            ...
            document.getElementById(idOfLightboxButton).addEventListener
          ) {
            document
    ...
    var RxpEmbedded = (function () {
      ...
      return {
        ...
        init: function (
          ...
          const embeddedInstance = RxpEmbedded.getInstance(serverSdkJson);
          ...
          // Sets the event listener on the PAY button. The click will invoke the embedded method
            ...
            document.getElementById(idOfEmbeddedButton).addEventListener
          ) {
            document
    ...
    var RxpRedirect = (function () {
      ...
      function init() {
        ...
        return {
          redirect: function () {
            const form = internal.createForm(document, token, true);
      ...
      return {
        ...
        init: function (idOfButton, merchantUrl, serverSdkJson) {
          ...
          const redirectInstance = RxpRedirect.getInstance(serverSdkJson);
  ...
  const RealexRemote = (function () {
     ...
     */
    const validateCardHolderName = function (cardHolderName) {
      // test for undefined
      if (!cardHolderName) {
        return false;
      }
     ...
     * characters.
     */
    const validateCvn = function (cvn) {
     ...
     * characters.
     */
    const validateAmexCvn = function (cvn) {
     ...
     * be between 1 and 12.
     */
    const validateExpiryDateFormat = function (expiryDate) {
     ...
     */
    const validateExpiryDateNotInPast = function (expiryDate) {
      // test valid format
